...
% representative Cross Information Potential (rCIP) Cluster Evaluation 
% Function (CEF).
%
% REFERENCES
...
classdef CVI_rCIP 
    properties (Access = public)       
        ...
        const = 0;                  % constant term of the Gaussian PDF
    ...
    methods 
            ...
            if label > obj.nClusters    
                ...
                if obj.nClusters == 0
                    ...
                    for jx=inds
                        ...
                        d_row_new(jx) = obj.const*(1/sqrt(det(Sigma_q)))*exp(-0.5*diff_m*inv(Sigma_q)*diff_m');                                        
                    end    
        ...
        function obj = param_batch(obj, data, labels)
            ...
            for ix=1:(obj.nClusters-1)
                for jx=ix+1:obj.nClusters
                    ...
                    obj.D(ix, jx) = obj.const*(1/sqrt(det(Sigma_q)))*exp(-0.5*diff_m*inv(Sigma_q)*diff_m');
                end
            end            
            obj.D = obj.D + obj.D';
        end         
        ...
        function obj = evaluate(obj) 
            ...
            values = obj.D(mask);
            obj.CriterionValue = sum(values); % rCIP/CEF index value
        end   
    end
end