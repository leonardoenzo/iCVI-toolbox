...
% index 43 (GD43) Cluster Validity Index.
%
% REFERENCES
...
classdef CVI_GD43 
    properties (Access = public)       
        ...
        CriterionValue = [];        % calculated GD43 index
    end 
    methods 
        ...
        function obj = param_inc(obj, sample, label)
            ...
            if label > obj.nClusters    
                ...
                if obj.nClusters == 0
                    ...
                    for jx=inds   
                        d_row_new(jx) = sqrt(sum((v_new - obj.v(jx, :)).^2, 2));
                    end    
        ...
        function obj = param_batch(obj, data, labels)  
            ...
            for ix=1:(obj.nClusters-1)
                for jx=ix+1:obj.nClusters
                    obj.D(ix, jx) = sqrt(sum((obj.v(ix, :) - obj.v(jx, :)).^2, 2));                    
                end
            end
            obj.D = obj.D + obj.D';              
        end         
        ...
        function obj = evaluate(obj)
            ...
            if obj.nClusters > 1        
                ...
                obj.CriterionValue = obj.Inter/obj.Intra; % GD43 index value  
            end            
        end   
    end
end